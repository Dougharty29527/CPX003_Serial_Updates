...
class OverfillCondition(AlarmCondition):
    ...
    def check(self, context: Alarm) -> bool:
        ...
        # Read TLS pin directly from hardware (original I2C path)
        try:
            tls_active = context.app.io.pins['tls'].value
            if tls_active:
                context.app.io.stop_cycle()
                context.repository.save_last_overfill_time(current_time)
                return True
        except Exception as e:
            Logger.error(f'AlarmManager: Error reading TLS pin: {e}')
        
        # ==================================================================
        # REV 10 PATCH: Fallback to ESP32 serial overfill data when I2C
        # hardware is unavailable (MCP23017 no longer on bus).
        # ESP32 reads GPIO38 with 8/5 hysteresis validation and sends
        # overfill status in every 15-second status packet.
        # This check runs AFTER the I2C attempt above, so if I2C works
        # (backward compatible), it takes priority.
        # ==================================================================
        try:
            serial_mgr = context.app.serial_manager if hasattr(context.app, 'serial_manager') else None
