...
class SerialManager:
    ...
    def __init__(self, data_handler):
        ...
        # =====================================================================
        # REV 10 PATCH: ESP32 sensor data received via serial (replaces I2C ADC)
        # The ESP32 now reads the ADS1015 ADC directly and sends values in its
        # 15-second status JSON. These are the latest received values.
        # =====================================================================
        ...
        # REV 10 PATCH: Mode-change detection for immediate serial relay control.
        # The ModeManager mmap file is updated by IOManager (even in child processes).
        # We poll it every 0.5s and send the mode to ESP32 immediately on change.
    ...
    def receive_esp32_status(self):
        ...
        try:
            ...
            # =================================================================
            # REV 10 PATCH: Parse sensor data from ESP32 status packet.
            # ESP32 reads the ADS1015 at 60Hz with rolling averages and sends
            # computed values. These replace direct I2C ADC reads from Linux.
            # =================================================================
            if 'pressure' in data:
                try:
            ...
            # REV 10 PATCH: Overfill from ESP32 GPIO38 with hysteresis validation.
            # Replaces direct MCP23017 pin 8 (TLS) read over I2C.
            if 'overfill' in data:
                try:
            ...
            # REV 10 PATCH: Profile and failsafe status from ESP32
            if 'profile' in data:
    ...
    # -------------------------------------------------------------------------
    # REV 10 PATCH: Immediate Mode Relay Control via Serial
    # -------------------------------------------------------------------------
    # The ESP32 no longer emulates an MCP23017 on I2C. Instead, it accepts
    # mode commands over serial and controls relays directly. These methods
    # detect mode changes from the ModeManager mmap file (written by child
    # processes) and send them to ESP32 immediately — no I2C needed.
    #
    ...
    def send_mode_immediate(self, mode_num):
        ...
        REV 10 PATCH: This is called when a mode change is detected via
        ModeManager polling. The ESP32 sets physical relay outputs based
        on the mode number — no MCP23017 I2C needed.
    ...
    def send_shutdown_command(self):
        ...
        REV 10 PATCH: Replaces I2C MCP23017 pin 4 write for shutdown relay.
        ESP32 recognizes {"mode":"shutdown"} as the ONLY way to trigger
        DISP_SHUTDN LOW. The pin starts HIGH on every ESP32 boot.
    ...
    def _mode_check_cycle(self, *args):
        ...
        REV 10 PATCH: Runs every 0.5 seconds via Kivy Clock. Reads the
        ...
        This bridges the gap between multiprocessing pin writes (now on
        MockPins) and the real serial relay control on ESP32.
        '''
        if self.ppp_active:
    ...
    def start(self):
        ...
        # REV 10 PATCH: Poll ModeManager every 0.5s for immediate relay control.
    ...
    def stop(self):
        ...
        # REV 10 PATCH: Stop mode polling
