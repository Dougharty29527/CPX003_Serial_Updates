# Python Control Panel — Changes Summary (Rev 10.7)
...
| Rev 10.6 | 2/6/2026 | Fix "CHECK I/O BOARD CONNECTION" false alarm — `hardware_available` set True. Add `send_normal_command()` for clearing 72-hour shutdown via `{"mode":"normal"}`. |
| Rev 10.7 | 2/9/2026 | **Pressure calibration (3 Python files).** `modem.py`: Added `send_calibration_command()` (sends `{"type":"cmd","cmd":"cal"}`), added `ps_cal` response parser (saves ESP32 calibration result to `gm_db` as `adc_zero`). `contractor_screen.py`: Fixed crash — replaced I2C `pressure_sensor.calibrate` call with serial `_send_calibration_to_esp32()`. Updated dialog text for altitude. ESP32 returns `{"type":"data","ps_cal":964.50}` after calibration. |
