...
class SerialManager:
    ...
    def __init__(self, data_handler):
        ...
        # sends computed values in fast sensor packets (5Hz). These variables
        ...
        self.esp32_relay_mode = 0        # Current relay mode (0-9) from ESP32
        self.esp32_shutdown = False      # True when ESP32 is in shutdown mode (bool)
    ...
    def receive_esp32_status(self):
        ...
        try:
            ...
            # =================================================================
            # FAST SENSOR PACKETS (5Hz/200ms): Parse real-time ESP32 data
            ...
            # SERIAL-ONLY: Relay mode, profile and failsafe status from ESP32
            if 'relayMode' in data:
                try:
                    self.esp32_relay_mode = int(data['relayMode'])
                except (ValueError, TypeError):
                    pass
            ...
            if 'shutdown' in data:
                try:
                    self.esp32_shutdown = bool(int(data['shutdown']))
                except (ValueError, TypeError):
                    pass
            ...
            if 'ps_cal' in data:
                try:
                    ...
                    if hasattr(self, 'data_handler') and self.data_handler:
                        try:
                            self.data_handler.save_calibration(cal_value)
                            self._log('info', f'Saved ESP32 calibration to database: {cal_value}')
                        except Exception as e:
                            self._log('error', f'Failed to save ESP32 calibration to database: {e}')
                except (ValueError, TypeError) as e:
                    self._log('warning', f'Invalid ps_cal value from ESP32: {data.get("ps_cal")} - {e}')
            ...
            if 'backfill' in data:
                try:
                    ...
                    self._log('error', f'Error processing backfill data: {e}')
            ...
            # Log received fields (only what's actually in this packet)
            # Fast sensor packets (5Hz) contain: pressure, current, overfill, sdcard, relayMode, failsafe, shutdown
            # Cellular packets (~10s) contain: datetime, lte, rsrp, rsrq, passthrough, operator, band, etc.
            received_fields = ', '.join(f'{k}={v}' for k, v in data.items())
            self._log('debug', f'ESP32 packet: {received_fields}')

            # Also log key sensor values for monitoring
            if any(key in data for key in ['pressure', 'current', 'overfill', 'relayMode', 'failsafe']):
                self._log('debug', f'ESP32 sensors: pressure={self.esp32_pressure:.2f} IWC, '
                                  f'current={self.esp32_current:.2f} A, overfill={self.esp32_overfill}, '
                                  f'mode={self.esp32_relay_mode}, failsafe={self.esp32_failsafe}, '
                                  f'shutdown={self.esp32_shutdown}')
    ...
    def send_fast_poll_command(self, enable=True):
        '''
        Send fast polling command to ESP32.

        Enables/disables faster BlueCherry polling (every 15 seconds instead of
        scheduled intervals). Fast polling automatically times out after 60 minutes.

        Args:
            enable (bool): True to enable fast polling, False to disable

        Example:
            send_fast_poll_command(True)   # Enable 15s polling for 60min max
            send_fast_poll_command(False)  # Return to scheduled polling
        '''
        if self.ppp_active or self.is_passthrough_active():
            self._log('warning', 'Cannot change polling during PPP/passthrough')
            return False
        ...
        try:
            ...
            msg = json.dumps({'type': 'cmd', 'cmd': 'fast_poll', 'val': val}, separators=(',', ':'))
            self.serial_port.write((msg + '\n').encode('ascii'))
            self.serial_port.flush()
            action = "enabled" if enable else "disabled"
            self._log('info', f'Fast polling {action} — ESP32 will {"poll every 15s" if enable else "return to scheduled polling"}')
            return True
        except Exception as e:
            self._log('error', f'Error sending fast poll command: {e}')
            return False

    def send_failsafe_command(self, enable=True):
        '''
        Send failsafe enable/disable command to ESP32.

        Controls whether ESP32 can enter autonomous failsafe mode when serial
        watchdog triggers. When disabled, watchdog only pulses GPIO39 every 10min.
        When enabled, watchdog pulses every 30min and can enter failsafe after 2 attempts.

        Args:
            enable (bool): True to enable failsafe capability, False to disable

        Example:
            send_failsafe_command(True)   # Allow ESP32 to enter failsafe if watchdog triggers
            send_failsafe_command(False)  # Disable failsafe, watchdog only pulses GPIO39
        '''
        if self.ppp_active or self.is_passthrough_active():
            self._log('warning', 'Cannot change failsafe during PPP/passthrough')
            return False
        ...
        try:
            cmd = 'enable_failsafe' if enable else 'exit_failsafe'
            msg = json.dumps({'type': 'cmd', 'cmd': cmd}, separators=(',', ':'))
            self.serial_port.write((msg + '\n').encode('ascii'))
            self.serial_port.flush()
            action = "enabled" if enable else "disabled"
            self._log('info', f'Failsafe {action} — ESP32 {"can enter" if enable else "cannot enter"} autonomous failsafe mode')
            return True
        except Exception as e:
            self._log('error', f'Error sending failsafe command: {e}')
            return False
